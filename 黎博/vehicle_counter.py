...
class Vehicle(object):
    def __init__(self, id, position):
        ...
        self.move_step = [] # vihicle's moving step, initialize with empty, first step is from position 1 to 2 
    ...
    def draw(self, original_frame):
        ...
        for point in self.positions:
            ...
            #cv2.putText(original_frame, str(self.id), point, cv2.FONT_HERSHEY_PLAIN, 0.7, (127, 255, 255), 1)
...
class VehicleCounter(object):
    ...
    @staticmethod
    def update_single_vehicle(vehicle, calculated_points, time_since_last_frame):
    ...
    def update_all_vehicles(self, calculated_points, time_since_last_frame):
        for v in self.vehicles:
            self.update_single_vehicle(v, calculated_points, time_since_last_frame)
        ...
        for vehicle in self.vehicles:
            if not vehicle.counted:
                if indetector(self.detector[0], vehicle.positions[-1]):
                    ...
                    print('{}:{}'.format(vehicle.id, vehicle.positions[-1]))               
                    self.r2 += 1
                    vehicle.counted = True
    ...
    def draw_all_vehicles(self, original_frame):
        # draw the vehicles on an image
        if (original_frame is not None):
            for vehicle in self.vehicles:
                vehicle.draw(original_frame)
    ...
    def find_if_a_new_vehicle(obj, calculated_points):
        ...
        if len(calculated_points) > 0:
            ...
            # no 'new points' means no old points, therefore, all object are new vehicles
            return 'yes'
    
    def creat_new_vehicles(self, new_objs, slope, b):
        # Add new vehicles based on the remaining matches (at first frame, all matches is new and will be added)
        for centroid in new_objs:
            x, y = centroid.ravel()
            # kill new object in detecting area:
            if indetector(self.detector[0], (x, y)) or indetector(self.detector[1], (x, y)) or indetector(self.detector[2], (x, y)) or indetector(self.detector[3], (x, y)):
                continue
            ...
            new_vehicle = Vehicle(self.next_vehicle_id, (x, y)) # vehicle ID starts from 1, then increase by 1 for next vehicle
